...
struct EmojiMemoryThemeEditor: View {
    ...
    var body: some View {
        NavigationView {
            ...
            .navigationBarItems(
                leading: Button(action: cancel, label: { Text("Cancelar") }),
                trailing: Button(action: saveTheme, label: { Text("Pronto") })
                    .disabled(cannotSave)
            )
        }
    }
    ...
    var nameSection: some View {
        Section {
            TextField("Nome do Tema", text: $name)
        }
    }
    ...
    var cardCountSection: some View {
        Section(header: Text("Quantidade de Cards").font(.headline)) {
            Stepper("\(numberOfPairs) pares", value: $numberOfPairs, in: min(emojis.count, 2)...emojis.count)
    ...
    var colorSection: some View {
        Section(header: Text("Cor").font(.headline)) {
